...
<div class="hidden sm:block">
    <x-bladewind::dropmenu modular="true">
        <x-slot:trigger>
            <span class="text-sm border border-transparent text-white/50 bg-white/10 hover:border hover:border-white/15 py-1 px-2 rounded-full">{{\Composer\InstalledVersions::getPrettyVersion('bladewindui/ui')}}
            <x-bladewind::icon name="chevron-down" class="!size-4" />
            </span>
        </x-slot:trigger>
        <x-bladewind::dropmenu.item>{{\Composer\InstalledVersions::getPrettyVersion('bladewindui/ui')}}</x-bladewind::dropmenu.item>
        <x-bladewind::dropmenu.item onclick="location.href='https://v4.bladewindui.com/install'">4.x</x-bladewind::dropmenu.item>
