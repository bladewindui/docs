<x-app>
    ...
    <p>The <code class="inline">view</code> attribute controls whether Calendar shows a whole month or a single week. It accepts <code class="inline">month</code> or <code class="inline">week</code>. If you do not set it, Calendar shows month view first, and the two buttons in the header let a visitor switch back and forth whenever they like. The <code class="inline">date</code> attribute tells Calendar which day to center the view on. It should be written as <code class="inline">Y-m-d</code>, for example <code class="inline">2026-08-14</code>, and it defaults to today's date if you leave it out. In month view, Calendar shows the whole month that <code class="inline">date</code> falls in. In week view, Calendar shows the seven days of the week that <code class="inline">date</code> falls in.</p>
    <p>Week view is not simply a shorter version of month view. It is a full hour by hour schedule, similar to the week view you would see in Outlook or Google Calendar. There is more about how that works a little further down this page, in the Week View section.</p>
    ...
    <p>Week view does not scroll to the very top of the day when it opens, because very few meetings happen at midnight and scrolling past several empty hours to find the working day would be annoying. Instead it opens already scrolled down to a reasonable hour in the morning, so the events people actually care about are visible right away.</p>
    ...
    <p>Calendar keeps the same overall height no matter what you are looking at, and it does this by default without you needing to configure anything. Left to its own devices, a calendar's size would naturally change all the time: some months have four weeks, some have five, some have six, and week view is much shorter than month view because it only shows one row of days. Without a fixed height, switching between any of these would make the whole calendar grow or shrink, which looks jumpy and can shift everything else around it on the page.</p>
    <p>To avoid that, Calendar reserves enough room for the largest case it will ever need, a six week month, which comes out to <code class="inline">40rem</code> by default. If what is currently showing needs less room than that, for example a shorter month or a single week, the extra space is simply left empty at the bottom rather than the calendar shrinking to fit. If what is showing needs more room than that, the calendar adds its own scrollbar inside the grid rather than growing past the height you asked for. You can set your own value with the <code class="inline">height</code> attribute, for example <code class="inline">28rem</code>, if the default is not right for your layout. If you would rather Calendar sized itself naturally to whatever it is showing, with no fixed height at all, pass an empty value like <code class="inline">height=""</code>.</p>
    ...
    <p>The Previous, Next, and Today buttons in the header, along with Page Up and Page Down on the keyboard, move Calendar to a different month or week. By default this all happens instantly in the browser, using the same list of events you already gave it, without needing to reload the page or wait on a request to your server. If you would rather have your own server decide what to show next instead, for example because you are loading a very large or constantly changing set of events, set <code class="inline">client-navigation="false"</code>. With that turned off, navigating only sends out the <code class="inline">before-navigate</code> and <code class="inline">navigate</code> events described below, and your application is responsible for showing the new month or week, whether that means loading a fresh page or updating things yourself with Livewire or Inertia. Data Grid offers this same choice for its own sorting and searching, if you have used that component before.</p>

    <h2 id="keyboard">Keyboard Interaction</h2>
    <p>Calendar's grid follows the same accessible pattern used elsewhere in this library: a single Tab stop gets a visitor into the grid, and from there the arrow keys move around inside it, rather than needing to Tab through every single day one at a time. In week view, the same keys move between the day headers running along the top of the week rather than between day cells, since week view no longer has day cells in the month view sense.</p>
    <x-bladewind::table><x-slot:header><th>Key</th><th>Action</th></x-slot:header>
        ...
        <tr><td><kbd>Page Up</kbd> / <kbd>Page Down</kbd></td><td>Go to the previous or next month, or the previous or next week while in week view.</td></tr>
        <tr><td><kbd>Shift</kbd> + <kbd>Page Up</kbd> / <kbd>Page Down</kbd></td><td>Go to the previous or next year, or the previous or next month while in week view.</td></tr>
        <tr><td><kbd>Enter</kbd> / <kbd>Space</kbd></td><td>Select the day currently focused, if selection is turned on.</td></tr>
    </x-bladewind::table>
    <p>Every event marker, including the timed events placed on week view's hour by hour grid, is a genuine link or button that can be reached with an ordinary Tab press and activated like any other control on the page. Nothing here is a decoration that only responds to a mouse.</p>
    ...
    <x-bladewind::table><x-slot:header><th>Event suffix</th><th>When it runs</th></x-slot:header>
        <tr><td><code class="inline">before-navigate</code>, <code class="inline">navigate</code></td><td>Just before, and then just after, the visible month or week changes.</td></tr>
        <tr><td><code class="inline">before-view-change</code>, <code class="inline">view-change</code></td><td>Just before, and then just after, switching between month view and week view.</td></tr>
    ...
    <x-bladewind::table><x-slot:header><th>Attribute</th><th>Default</th><th>Description</th></x-slot:header>
        ...
        <tr><td>date</td><td>today</td><td>Anchor date (Y-m-d) for the month or week initially shown.</td></tr>
...
calendarSelectedDates('team-calendar'); // ['2026-08-14']</code></pre>
    ...
    <x-slot:side_nav>
        ...
        <div class="flex items-center"><div class="dot"></div><a href="#week-view">Week view</a></div>
